...
func writeContent(ds *gadget.OnDiskStructure, gadgetRoot string, intercept gadget.FileActionInterceptor) error {
	switch {
	case !ds.IsPartition():
	...
	case ds.HasFilesystem():
		if err := writeFilesystemContent(ds, gadgetRoot, intercept); err != nil {
			return err
		}
	}
...
func writeFilesystemContent(ds *gadget.OnDiskStructure, gadgetRoot string, intercept gadget.FileActionInterceptor) (err error) {
	...
	fs, err := gadget.NewMountedFilesystemWriter(gadgetRoot, &ds.LaidOutStructure, intercept)
	if err != nil {
