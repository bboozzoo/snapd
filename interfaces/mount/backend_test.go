...
type backendSuite struct {
	...
	iface3 *ifacetest.TestDelayedEffectApplyingInterface
}
...
func (s *backendSuite) SetUpTest(c *C) {
	...
	s.iface3 = &ifacetest.TestDelayedEffectApplyingInterface{
		TestInterface: ifacetest.TestInterface{
			InterfaceName: "iface3",
		},
		SupportsDelayedEffectCallback: nil,
	}
...
func (s *backendSuite) TestSetupDelaysIfDuringOtherUpdateAndConnectedOnPlugSideAndSupported(c *C) {
	...
	supportsCalled := false
	s.iface3.SupportsDelayedEffectCallback = func(backend interfaces.SecuritySystem) bool {
		supportsCalled = true
		c.Check(backend, Equals, interfaces.SecurityMount)
		return true
	}
	...
	c.Assert(err, IsNil)
	c.Check(supportsCalled, Equals, true)
...
func (s *backendSuite) TestEffectNotDelayedIfConnectedOnPlugAndOwnUpdateAndSupported(c *C) {
...
func (s *backendSuite) TestEffectNotDelayedIfConnectedOnSlotAndSupported(c *C) {
...
func (s *backendSuite) TestEffectNotDelayedNoSupportedByInterface(c *C) {
	const mockImmediateProducerSnapYaml = `name: producer
version: 1
slots:
    iface-slot:
        interface: iface
`
	const mockImmediateConsumerSnapYaml = `name: consumer
version: 1
plugs:
    iface-plug:
        interface: iface
`
	...
	// delaying effects possible, but not supported by the connected interface
	producerAppSet := s.AddSnap(c, "producer", mockImmediateProducerSnapYaml, 0)
	consumerAppSet := s.AddSnap(c, "consumer", mockImmediateConsumerSnapYaml, 0)
