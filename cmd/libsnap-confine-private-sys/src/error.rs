...
#[no_mangle]
pub unsafe extern "C" fn sc_error_new(
    domain: *const c_char,
    code: c_int,
    msgfmt: *const c_char,
) -> *mut sc_error {
    ...
    let err = Box::new(new(s_domain, code, s_msg));
    Box::into_raw(err)
}
...
impl sc_error {
    ...
    pub fn into_boxed_ptr(self) -> *mut Self {
        Box::into_raw(self.into_boxed())
    }
}

#[no_mangle]
pub unsafe extern "C" fn sc_error_free(self_err: *mut sc_error) {
    if self_err.is_null() {
        return;
    }
...
#[no_mangle]
pub unsafe extern "C" fn sc_error_domain(self_err: *const sc_error) -> *const c_char {
    unsafe {
        if self_err.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn sc_error_code(self_err: *const sc_error) -> c_int {
    unsafe {
        if self_err.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn sc_error_msg(self_err: *const sc_error) -> *const c_char {
    unsafe {
        if self_err.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn sc_error_forward(
...
#[no_mangle]
pub unsafe extern "C" fn sc_error_match(
    self_err: *const sc_error,
    domain: *const c_char,
    code: c_int,
) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn sc_die_on_error(self_err: *const sc_error) {
    if !self_err.is_null() {
        unsafe {
