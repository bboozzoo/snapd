 ...
 **/
static void __attribute__((sentinel)) test_argc_argv(int *argcp, char ***argvp, ...) {
    int argc = 0;
    char **argv = NULL;
...
static void test_sc_nonfatal_parse_args__ubuntu_core_launcher(void) {
    ...
    int argc;
    char **argv;
    test_argc_argv(&argc, &argv, "/usr/bin/ubuntu-core-launcher", "snap.SNAP_NAME.APP_NAME", "snap.SNAP_NAME.APP_NAME",
                   "/usr/lib/snapd/snap-exec", "--option", "arg", NULL);
