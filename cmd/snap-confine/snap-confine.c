...
int main(int argc, char **argv) {
     ...
     * all permitted caps, but clear effective and inheritable. */
    ...
    // Temporarily drop all effective capabilities, since we don't need any for a while.
    debug("dropping caps");
    ...
    static const cap_value_t only_seccomp_caps[] = {
        CAP_SYS_ADMIN,  // seccomp
    };
    if (cap_set_flag(cap_only_sys_admin, CAP_EFFECTIVE, SC_ARRAY_SIZE(only_seccomp_caps), only_seccomp_caps, CAP_SET) !=
            0 ||
        cap_set_flag(cap_only_sys_admin, CAP_PERMITTED, SC_ARRAY_SIZE(only_seccomp_caps), only_seccomp_caps, CAP_SET) !=
            0) {
