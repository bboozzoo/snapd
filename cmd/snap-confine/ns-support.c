...
void sc_initialize_mount_ns(void) {
    ...
    for (struct sc_mountinfo_entry *entry = sc_first_mountinfo_entry(info); entry != NULL;
...
static dev_t find_base_snap_device(const char *base_snap_name, const char *base_snap_rev) {
    ...
    bool found = false;
    for (struct sc_mountinfo_entry *mie = sc_first_mountinfo_entry(mi); mie != NULL;
...
// unconditionally.
static int sc_inspect_and_maybe_discard_stale_ns(int mnt_fd, const char *snap_name, const char *base_snap_name,
                                                 int snap_discard_ns_fd, bool is_normal_mode) {
    ...
    sc_must_snprintf(fname, sizeof fname, "%s/%s/current", SNAP_MOUNT_DIR, base_snap_name);
    ...
    if (base_snap_rev[sizeof base_snap_rev - 1] != '\0') {
        die("cannot read current revision of snap %s: value too long", snap_name);
    }
    ...
    if (child == 0) {
        ...
        bool should_discard = is_normal_mode ? should_discard_current_ns(base_snap_dev) : false;

        // Send this back to the parent: 2 - discard, 1 - keep.
        // Note that we cannot just use 0 and 1 because of the semantics of eventfd(2).
        if (eventfd_write(event_fd, should_discard ? SC_DISCARD_YES : SC_DISCARD_NO) < 0) {
...
#define NSFS_MAGIC 0x6e736673
    ...
    if (ns_statfs_buf.f_type == NSFS_MAGIC || ns_statfs_buf.f_type == PROC_SUPER_MAGIC) {
        ...
        if (sc_inspect_and_maybe_discard_stale_ns(mnt_fd, snap_name, base_snap_name, snap_discard_ns_fd,
                                                  is_normal_mode) == EAGAIN) {
            return ESRCH;
        }
