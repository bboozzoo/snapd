    ...
    for (size_t i = 0; i < glob_res.gl_pathc; ++i) {
        ...
        switch (stat_buf.st_mode & S_IFMT) {
            case S_IFLNK:;
                ...
                num_read = readlink(pathname, hostfs_symlink_target, sizeof hostfs_symlink_target);
                if (num_read == -1) {
...
static void sc_probe_nvidia_driver(struct sc_nvidia_driver *driver) {
    ...
    if (fscanf(file, "%d.%d", &driver->major_version, &driver->minor_version) != 2) {
        die("cannot parse nvidia driver version string");
    }
    debug("parsed nvidia driver version: %d.%d", driver->major_version, driver->minor_version);
}
...
static int sc_mount_nvidia_is_driver_in_dir(const char *dir) {
    ...
    sc_must_snprintf(driver_path, sizeof driver_path, "%s/" LIBNVIDIA_GLCORE_SO_PATTERN, dir, driver.major_version,
