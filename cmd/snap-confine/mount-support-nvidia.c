...
//
// A missing or unreadable source is not treated as fatal: the export tree
// under /var/lib/snapd/export (see TODO_CORE_DRIVER_LIBS.md, "GC") is a
// mutable structure that snapd may rewrite or garbage-collect concurrently
// with this code running, so src having disappeared since it was discovered
// by the caller (via glob() or a manifest read) is an expected, recoverable
// condition, not a bug - a snap launch must not fail just because a driver
// snap was refreshed or disconnected at the wrong moment. Once open()
// succeeds, POSIX unlink semantics mean the fd keeps the inode alive for the
// rest of this function, so fstat() and sendfile() below cannot fail due to
// a concurrent removal of src - the only source-side race is in open()
// itself, and fstat() is guarded the same way purely for defense in depth.
//
...
void sc_mount_snap_gpu_driver(const char *rootfs_dir, const char *base_snap_name, sc_distro distro) {
    ...
    // does not depend on an NVIDIA kernel module being loaded at all. See
