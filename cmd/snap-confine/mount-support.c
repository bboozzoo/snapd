...
static void setup_private_mount(const char *snap_name) {
    ...
    if (pwd == NULL) die("cannot get current working directory");
    if (chdir("/") != 0) die("cannot change directory to '/'");
    ...
    if (chdir(pwd) != 0) die("cannot change current working directory to the original directory");
    free(pwd);
}
...
static void sc_bootstrap_mount_namespace(const struct sc_mount_config *config) {
    ...
    for (const struct sc_mount *mnt = config->mounts; mnt->path != NULL; mnt++) {
        if (mnt->is_bidirectional && mkdir(mnt->path, 0755) < 0 && errno != EEXIST) {
    ...
    if (config->normal_mode) {
        ...
        for (const char **dirs = dirs_from_core; *dirs != NULL; dirs++) {
            ...
            if (access(dir, F_OK) == 0) {
                ...
                if (lstat(src, &buf) == 0 && lstat(dst, &buf) == 0) {
...
void sc_populate_mount_ns(struct sc_apparmor *apparmor, int snap_update_ns_fd, const char *base_snap_name,
    ...
    if (is_normal_mode) {
        ...
        sc_must_snprintf(rootfs_dir, sizeof rootfs_dir, "%s/%s/current/", SNAP_MOUNT_DIR, base_snap_name);
        if (access(rootfs_dir, F_OK) != 0) {
            ...
            // still not ok, die
            if (access(rootfs_dir, F_OK) != 0) die("cannot locate the base snap: %s", base_snap_name);
        }
    ...
    if (chdir(vanilla_cwd) != 0) {
        debug("cannot remain in %s, moving to the void directory", vanilla_cwd);
