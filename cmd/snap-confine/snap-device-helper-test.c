...
static void my_unsetenv(const char *k) { g_unsetenv(k); }
...
static int run_sdh(gchar *action, gchar *appname, gchar *devpath, gchar *majmin) {
    ...
    gboolean ret = g_spawn_sync(NULL, (gchar **)argv->pdata, NULL, 0, NULL, NULL, NULL, NULL, &status, &err);
    g_free(mod_appname);
    g_free(sdh_path);
...
static void test_sdh_action(gconstpointer test_data) {
    ...
    gchar *with_data = g_build_filename(mock_dir, td->app, td->file_with_data, NULL);
    gchar *without_data = g_build_filename(mock_dir, td->app, td->file_with_no_data, NULL);
    gchar *data = NULL;
    ...
    int ret = run_sdh(td->action, td->app, "/devices/foo/block/sda/sda4", "8:4");
...
static struct sdh_test_data add_data = {"add", "snap.foo.bar", "snap_foo_bar", "devices.allow", "devices.deny"};
static struct sdh_test_data change_data = {"change", "snap.foo.bar", "snap_foo_bar", "devices.allow", "devices.deny"};
static struct sdh_test_data remove_data = {"remove", "snap.foo.bar", "snap_foo_bar", "devices.deny", "devices.allow"};
static struct sdh_test_data instance_add_data = {"add", "snap.foo_bar.baz", "snap_foo_bar_baz", "devices.allow",
                                                 "devices.deny"};
static struct sdh_test_data instance_change_data = {"change", "snap.foo_bar.baz", "snap_foo_bar_baz", "devices.allow",
                                                    "devices.deny"};
...
static struct sdh_test_data instance_add_hook_data = {"add", "snap.foo_bar.hook.configure",
                                                      "snap_foo_bar_hook_configure", "devices.allow", "devices.deny"};
...
static void __attribute__((constructor)) init(void) {
    g_test_add_data_func("/snap-device-helper/add", &add_data, test_sdh_action);
    g_test_add_data_func("/snap-device-helper/change", &change_data, test_sdh_action);
    g_test_add_data_func("/snap-device-helper/remove", &remove_data, test_sdh_action);
    g_test_add_func("/snap-device-helper/err", test_sdh_err);
    g_test_add_data_func("/snap-device-helper/parallel/add", &instance_add_data, test_sdh_action);
    g_test_add_data_func("/snap-device-helper/parallel/change", &instance_change_data, test_sdh_action);
    g_test_add_data_func("/snap-device-helper/parallel/remove", &instance_remove_data, test_sdh_action);
    // hooks
    g_test_add_data_func("/snap-device-helper/hook/add", &add_hook_data, test_sdh_action);
    g_test_add_data_func("/snap-device-helper/hook/parallel/add", &instance_add_hook_data, test_sdh_action);
    g_test_add_data_func("/snap-device-helper/hook-name-hook/parallel/add", &instance_add_instance_name_is_hook_data,
                         test_sdh_action);
}
