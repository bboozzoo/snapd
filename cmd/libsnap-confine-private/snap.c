...
bool verify_security_tag(const char *security_tag, const char *snap_name) {
    ...
    return len == strlen(snap_name) && strncmp(security_tag + matches[1].rm_so, snap_name, len) == 0;
}

bool sc_is_hook_security_tag(const char *security_tag) {
    const char *whitelist_re = "^snap\\.[a-z](-?[a-z0-9])*(_[a-z0-9]{1,10})?\\.(hook\\.[a-z](-?[a-z])*)$";
...
void sc_snap_name_validate(const char *snap_name, struct sc_error **errorp) {
    ...
    // Ensure that name is not NULL
    if (snap_name == NULL) {
        err = sc_error_init(SC_SNAP_DOMAIN, SC_SNAP_INVALID_NAME, "snap name cannot be NULL");
        goto out;
    }
    ...
    if (skip_one_char(&p, '-')) {
        err = sc_error_init(SC_SNAP_DOMAIN, SC_SNAP_INVALID_NAME, "snap name cannot start with a dash");
        goto out;
    }
    ...
    for (; *p != '\0';) {
        ...
        if (skip_one_char(&p, '-') > 0) {
            n++;
            if (*p == '\0') {
                err = sc_error_init(SC_SNAP_DOMAIN, SC_SNAP_INVALID_NAME, "snap name cannot end with a dash");
                goto out;
            }
            if (skip_one_char(&p, '-') > 0) {
    ...
    if (!got_letter) {
        err = sc_error_init(SC_SNAP_DOMAIN, SC_SNAP_INVALID_NAME, "snap name must contain at least one letter");
        goto out;
    }
    if (n < 2) {
        err = sc_error_init(SC_SNAP_DOMAIN, SC_SNAP_INVALID_NAME, "snap name must be longer than 1 character");
        goto out;
    }
    if (n > 40) {
        err = sc_error_init(SC_SNAP_DOMAIN, SC_SNAP_INVALID_NAME, "snap name must be shorter than 40 characters");
        goto out;
    }
