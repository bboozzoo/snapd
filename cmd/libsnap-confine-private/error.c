...
static struct sc_error *sc_error_initv(const char *domain, int code, const char *msgfmt, va_list ap) {
...
struct sc_error *sc_error_init_from_errno(int errno_copy, const char *msgfmt, ...) {
    va_list ap;
    va_start(ap, msgfmt);
    struct sc_error *err = sc_error_initv(SC_ERRNO_DOMAIN, errno_copy, msgfmt, ap);
    va_end(ap);
    return err;
}
