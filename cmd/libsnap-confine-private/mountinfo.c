 ...
 **/
static struct sc_mountinfo_entry *sc_parse_mountinfo_entry(const char *line) __attribute__((nonnull(1)));

/**
 * Free a sc_mountinfo structure and all its entries.
 **/
static void sc_free_mountinfo(struct sc_mountinfo *info) __attribute__((nonnull(1)));

/**
 * Free a sc_mountinfo entry.
 **/
static void sc_free_mountinfo_entry(struct sc_mountinfo_entry *entry) __attribute__((nonnull(1)));

struct sc_mountinfo_entry *sc_first_mountinfo_entry(struct sc_mountinfo *info) {
    return info->first;
}

struct sc_mountinfo_entry *sc_next_mountinfo_entry(struct sc_mountinfo_entry *entry) {
    return entry->next;
}
...
static void show_buffers(const char *line, int offset, struct sc_mountinfo_entry *entry) {
    ...
    fputc('>', stderr);
    for (int i = 0; i < strlen(line); ++i) fputc('=', stderr);
    fputc('<', stderr);
    fputc('\n', stderr);
#endif  // MOUNTINFO_DEBUG
}

static char *parse_next_string_field(struct sc_mountinfo_entry *entry, const char *line, int *offset) {
    int offset_delta = 0;
...
static struct sc_mountinfo_entry *sc_parse_mountinfo_entry(const char *line) {
    ...
    struct sc_mountinfo_entry *entry = calloc(1, sizeof *entry + strlen(line) + 1);
    if (entry == NULL) {
        return NULL;
    }
#ifdef MOUNTINFO_DEBUG
    ...
    if ((entry->super_opts = parse_next_string_field(entry, line, &offset)) == NULL) goto fail;
    show_buffers(line, offset, entry);
    return entry;
fail:
    free(entry);
...
static void sc_free_mountinfo_entry(struct sc_mountinfo_entry *entry) { free(entry); }
