...
func symlinkTargetCheck(filename string, expectedTarget interface{}, exact bool) (result bool, error string) {
	...
	if err != nil {
		return false, fmt.Sprintf("Cannot read symbolic link target %q: %v", filename, err)
	}
	if exact {
		...
		case fmt.Stringer:
			result = target == expectedTarget.String()
		default:
		...
		case fmt.Stringer:
			result = strings.Contains(target, expectedTarget.String())
		default:
			error = fmt.Sprintf("Cannot compare symbolic link target with something of type %T", expectedTarget)
		}
	}
