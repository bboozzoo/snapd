...
func (s *servicesTestSuite) testStopServiceEndure(c *C, assertStrings [][]string) {
...
func (s *servicesTestSuite) TestStopServiceEndureLegacySystemd(c *C) {
	releaseRestore := release.MockOnClassic(true)
	defer releaseRestore()
...
func (s *servicesTestSuite) TestStopServiceEndureModernSystemd(c *C) {
	releaseRestore := release.MockOnClassic(false)
	defer releaseRestore()
	...
	s.testStopServiceEndure(c,
		[][]string{
			{"show", "--property=Id,ActiveState,UnitFileState,Type,Names,NeedDaemonReload", "snap.survive-snap.survivor.service"},
		},
	)
}

func (s *servicesTestSuite) testStopServiceSigs(c *C, assertStrings [][]string) {
...
func (s *servicesTestSuite) TestStopServiceSigsLegacySystemd(c *C) {
	releaseRestore := release.MockOnClassic(true)
	defer releaseRestore()
...
func (s *servicesTestSuite) TestStopServiceSigsModernSystemd(c *C) {
	releaseRestore := release.MockOnClassic(false)
	defer releaseRestore()
	...
	s.testStopServiceSigs(c,
		[][]string{
			{"show", "--property=Id,ActiveState,UnitFileState,Type,Names,NeedDaemonReload", "snap.survive-snap.srv.service"},
		},
	)
}
...
`, &snap.SideInfo{Revision: snap.R(12)})
	...
	calls := 0
	r := systemd.MockSystemctl(func(cmd ...string) ([]byte, error) {
		var numFailedCalls int
		if "ubuntu-core" == release.ReleaseInfo.ID && release.ReleaseInfo.VersionID == "20" {
			numFailedCalls = 2 // show and daemon-reload
		} else {
			numFailedCalls = 1
		}
		if calls < numFailedCalls {
...
func (s *servicesTestSuite) TestFailedAddSnapCleansUpLegacySystemd(c *C) {
	releaseRestore := release.MockOnClassic(true)
	defer releaseRestore()
	releaseRestore = release.MockReleaseInfo(&release.OS{ID: "ubuntu", VersionID: "14.04"})
	defer releaseRestore()
...
func (s *servicesTestSuite) TestFailedAddSnapCleansUpModernSystemd(c *C) {
	releaseRestore := release.MockOnClassic(false)
	defer releaseRestore()
	releaseRestore = release.MockReleaseInfo(&release.OS{ID: "ubuntu-core", VersionID: "20"})
	defer releaseRestore()
...
func (s *servicesTestSuite) TestStopAndDisableServicesLegacySystemd(c *C) {
	releaseRestore := release.MockOnClassic(true)
	defer releaseRestore()
	releaseRestore = release.MockReleaseInfo(&release.OS{ID: "ubuntu", VersionID: "14.04"})
	defer releaseRestore()
	s.testStopAndDisableServices(c)
}

func (s *servicesTestSuite) TestStopAndDisableServicesModernSystemd(c *C) {
	releaseRestore := release.MockOnClassic(false)
	defer releaseRestore()
	releaseRestore = release.MockReleaseInfo(&release.OS{ID: "ubuntu-core", VersionID: "20"})
	defer releaseRestore()
