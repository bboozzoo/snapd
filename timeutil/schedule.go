...
type Schedule struct {
	...
	// when the day-of-the-month schedule ends, can be set to
...
func (sched *Schedule) String() string {
	...
	if sched.Weekday != "" {
		fmt.Fprintf(buf, "%s", sched.Weekday)
		if sched.MonthWeekday != monthWeekdayNone {
			fmt.Fprintf(buf, "%s", sched.MonthWeekday)
		}
	}
	if sched.WeekdayEnd != "" {
		fmt.Fprintf(buf, "-%s", sched.WeekdayEnd)
		mwe := sched.MonthWeekdayEnd
		if mwe == monthWeekdayNone && sched.MonthWeekday != monthWeekdayNone {
			mwe = sched.MonthWeekdayEnd
		}
		if mwe != monthWeekdayNone {
			fmt.Fprintf(buf, "%s", mwe)
		}
	}
...
// Generate a per day schedule
func makeWeekdayEvent(start string, end string) *Schedule {
...
// Split a span string `<start>-<end>` and return the start and end pieces. If
// string is not a span then the whole string is returned
...
func parseEventSetScheduleV2(s string) ([]*Schedule, error) {
	...
	for _, event := range events {
		...
		if isValidTime(start) && isValidTime(end) {
			...
			tempScheds = append(tempScheds, makeWeekdayEvent(start, end))
		} else {
			// no, it's an error
