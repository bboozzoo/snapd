...
func (m *InterfaceManager) setupProfilesForAppSet(
	...
	for _, name := range affectedNames[1:] {
		...
		// TODO: the safe bet right now is to only allow side affects when the
		// snap is on the plug side of the connection
		if sctx.Reason == interfaces.SnapSetupReasonConnectedSlotProviderUpdate {
			// fornewly established connections, especially ones with plug side
			...
			if sctx.CanDelayEffects {
				...
				sctx.DelayEffect = func(backend interfaces.SecurityBackend, item interfaces.DelayedSideEffect) {
					if !interfaces.SupportsDelayingEffects(backend) {
						delayErr = fmt.Errorf("internal error: attempt to delay effects for backend %q without support for it",
							backend.Name())
						return
					}
...
// delayedSnapsWork returns delayed backed effects.
...
func (m *InterfaceManager) doDelayedBackendSideEffects(task *state.Task, _ *tomb.Tomb) error {
	//  Single catch all task handler for all delayed side effects for
	// snaps. The identifies task which snaps are affected, and whether the
	// snaps that triggered the side effect were successfully processed. It
	// then schedules new tasks, one for each affected snap to apply them.
	...
	var snapLanes []int
	if err := task.Get("monitor-lanes", &snapLanes); err != nil {
		return fmt.Errorf("internal error: monitored lanes not set at the task level: %w", err)
	}
	...
	// TODO:deferred-mount-ns-update lanes name original snaps that are
	// installing/updating, while delayed work references affected snaps
	...
	for triggeredBySnap, affectedSnaps := range delayed.TriggeringSnaps {
		...
		if _, ok := successfulTriggeringSnaps[triggeredBySnap]; !ok {
			logger.Debugf("skipping effects triggered by of a failed %q snap", triggeredBySnap)
			continue
		}
	...
	for affectedSnap, backendEffects := range snapsWithDelayedEffects {
		...
		// TODO:deferred-mount-ns-update: inherit HaltTasks?
		ts.WaitFor(task)
	}
