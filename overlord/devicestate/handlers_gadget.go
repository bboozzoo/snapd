...
func (m *DeviceManager) doUpdateGadgetAssets(t *state.Task, _ *tomb.Tomb) error {
	...
	interceptManagedBootAssetsUpdate = boot.InterceptManagedAssetsUpdate(model)
	st.Unlock()
	err = gadgetUpdate(*currentData, *updateData, snapRollbackDir, updatePolicy, interceptManagedBootAssetsUpdate)
	st.Lock()
	if err != nil {
