...
func (s *deviceMgrSuite) TestDeviceManagerEnsurePostFactoryResetEncrypted(c *C) {
	...
	factoryResetMarkercontent := []byte(`{"fallback-save-key-hash":"d192153f0a50e826c6eb400c8711750ed0466571df1d151aaecc8c73095da7ec104318e7bf74d5e5ae2940827bf8402b"}
`)
	c.Assert(ioutil.WriteFile(filepath.Join(dirs.SnapDeviceDir, "factory-reset"), factoryResetMarkercontent, 0644), IsNil)

	completeCalls := 0
	restore := devicestate.MockBootCompleteFactoryReset(func(encrypted bool) error {
		completeCalls++
...
func (s *deviceMgrSuite) TestDeviceManagerEnsurePostFactoryResetEncryptedError(c *C) {
	...
	factoryResetMarkercontent := []byte(`{"fallback-save-key-hash":"uh-oh"}
`)
	c.Assert(ioutil.WriteFile(filepath.Join(dirs.SnapDeviceDir, "factory-reset"), factoryResetMarkercontent, 0644), IsNil)
