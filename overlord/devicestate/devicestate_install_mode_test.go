...
func (s *deviceMgrInstallModeSuite) doRunFactoryResetChange(c *C, model *asserts.Model, tc resetTestCase) error {
	...
	restore = devicestate.MockSecbootChangeEncryptionKey(func(node string, key keys.EncryptionKey) error {
		if tc.encrypt {
	...
	bootMakeBootableCalled := 0
	restore = devicestate.MockBootMakeSystemRunnableAfterReset(func(makeRunnableModel *asserts.Model, bootWith *boot.BootableSet, seal *boot.TrustedAssetsInstallObserver) error {
