...
func createSystemForModelFromValidatedSnaps(getInfo func(name string) (*snap.Info, bool, error), db asserts.RODatabase, label string, model *asserts.Model) (newFiles []string, dir string, err error) {
	...
	for _, sn := range localSnaps {
		fmt.Printf("derive side info: %q %+v\n", sn.Path, sn)
	...
	for {
		...
		fmt.Printf("to download should be empty: %v\n", toDownload)
		// for _, sn := range toDownload {
		// 	// this is unexpected, all snaps should be coming from
		// 	// local files at this point?

		// 	logger.Noticef("to download processing snap %+v", sn)
		// 	info := modelSnaps[sn.SnapName()]
		// 	if info == nil {
		// 		return nil, "", fmt.Errorf("internal error: seed snap %q not accounted for", sn.SnapName())
		// 	}
		// 	if info.ID() == "" {
		// 		// snap is unasserted, so treat is as a local one
		// 		logger.Noticef("snap %q is unasserted", sn.SnapName())
		// 		if err := w.SetLocalInfo(sn, info, info.MountFile()); err != nil {
		// 			return nil, "", err
		// 		}
		// 		// since the snap is unasserted there will be no
		// 		// assertion refs to set
		// 		continue
		// 	}
		// 	if err := w.SetInfo(sn, info); err != nil {
		// 		return nil, "", err
		...
		// 	// if snap is part of download during remodel, Channel
		// 	// comes from snapsup.Channel, otherwise it comes from
		// 	// actual state
		// 	if err := w.SetRedirectChannel(sn, info.Channel); err != nil {
		// 		return nil, "", err
		// 	}
		// 	// keep track of all refs so far, new ones belong to the
		// 	// snap we are processing
		// 	prev := len(f.Refs())
		// 	if err := snapasserts.FetchSnapAssertions(f, info.Sha3_384); err != nil {
		// 		return nil, "", fmt.Errorf("cannot fetch snap assertions for %q: %v", sn.SnapName(), err)
		// 	}
		// 	aRefs := f.Refs()[prev:]
		// 	sn.ARefs = aRefs
		// }
	...
	// XXX call w.Warnings(), store them in the task log
	...
	copySnap := func(name, src, dst string) error {
		fmt.Printf("copying seed snap %q from %v to %v\n", name, src, dst)
