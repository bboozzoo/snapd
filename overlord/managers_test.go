...
func verifyProcessDelayedEffectsTasks(c *C, ts *state.TaskSet) {
	c.Assert(ts.Tasks(), HasLen, 1)
	reRefresh := ts.Tasks()[0]
	c.Check(reRefresh.Kind(), Equals, "process-delayed-security-backend-effects")
	// nothing should wait on it
	c.Check(reRefresh.NumHaltTasks(), Equals, 0)
}

func verifyAtMostOneProcessDelayedEffects(c *C, tasks []*state.Task) {
	seen := false
	for _, t := range tasks {
