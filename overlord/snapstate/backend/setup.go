...
type SetupKernelSnapOptions struct {
	// Regenerate requests the check-and-conditionally-regenerate pass
	// (kernel.KernelDriversTreeOptions.Regenerate) against an
	// already-installed, currently active kernel+revision, instead of the
	// normal fresh-install behavior (which assumes destDir does not exist
	// yet and is a no-op if it does).
	Regenerate bool
}
