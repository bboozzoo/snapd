...
`
	...
	args, err := tg.CommandLine("snapd_recovery_mode=run", "", extraArgs)
	c.Assert(err, IsNil)
	c.Check(args, Equals, `snapd_recovery_mode=run console=ttyS0 console=tty1 panic=-1 systemd.debug-shell=1 rd.systemd.debug-shell=1 dangerous foo bar baz=1`)
	...
	c.Assert(err, IsNil)
	// static command line from recovery asset
	c.Check(args, Equals, `snapd_recovery_mode=recover snapd_recovery_system=20200202 console=ttyS0 console=tty1 panic=-1 systemd.debug-shell=1 rd.systemd.debug-shell=1 dangerous foo bar baz=1`)
}
