...
func (s *grubAssetsTestSuite) TestGrubConf(c *C) {
	s.testGrubConfigContains(c, "grub.cfg",
		"snapd_recovery_mode",
		"set snapd_static_cmdline_args='console=ttyS0 console=tty1 panic=-1 systemd.debug-shell=1 dangerous'",
	)
}
...
func (s *grubAssetsTestSuite) TestGrubCmdlineSnippetEditions(c *C) {
	for _, tc := range []struct {
		...
		{"grub.cfg:static-cmdline", 1, []byte("console=ttyS0 console=tty1 panic=-1 systemd.debug-shell=1 dangerous")},
		{"grub-recovery.cfg:static-cmdline", 1, []byte("console=ttyS0 console=tty1 panic=-1 systemd.debug-shell=1 dangerous")},
	} {
...
func (s *grubAssetsTestSuite) TestGrubCmdlineSnippetCrossCheck(c *C) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			content: []byte("console=ttyS0 console=tty1 panic=-1 systemd.debug-shell=1 dangerous"),
