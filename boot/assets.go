...
func (c *trustedAssetsCache) Add(blName, assetPath, assetName string) (ta *trackedAsset, added bool, err error) {
...
func NewTrustedAssetsInstallObserver(model *asserts.Model, gadgetDir string) (*TrustedAssetsInstallObserver, error) {
...
func (o *TrustedAssetsInstallObserver) Observe(op gadget.ContentOperation, affectedStruct *gadget.LaidOutStructure, root, realSource, relativeTarget string) (bool, error) {
	...
	if !strutil.ListContains(o.trustedAssets, relativeTarget) {
		// not one of the trusted assets
		return true, nil
	}
	ta, added, err := o.cache.Add(o.bl.Name(), realSource, filepath.Base(relativeTarget))
	if err != nil {
		return false, err
	}
	if added {
