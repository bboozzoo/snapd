...
func (s *assetsSuite) TestAssetsCacheAddDrop(c *C) {
	...
	// add a new file
	ta, added, err := cache.Add("grub", filepath.Join(d, "foobar"), "grubx64.efi")
	c.Assert(err, IsNil)
	...
	taAgain, added, err := cache.Add("grub", filepath.Join(d, "foobar"), "grubx64.efi")
	c.Assert(err, IsNil)
	// file already cached
	...
	taDifferentAsset, added, err := cache.Add("grub", filepath.Join(d, "foobar"), "bootx64.efi")
	c.Assert(err, IsNil)
	// new entry in cache
	c.Check(added, Equals, true)
	...
	taExistingAssetName, added, err := cache.Add("grub", filepath.Join(d, "foobar"), "bootx64.efi")
	c.Assert(err, IsNil)
	// new entry in cache
	c.Check(added, Equals, true)
...
func (s *assetsSuite) TestAssetsCacheAddErr(c *C) {
	...
	_, _, err = cache.Add("grub", filepath.Join(d, "no-file"), "grubx64.efi")
	...
	_, _, err = cache.Add("grub", filepath.Join(d, "foobar"), "grubx64.efi")
...
func (s *assetsSuite) TestAssetsCacheDropErr(c *C) {
	...
	c.Assert(err, IsNil)
	// cannot create bootloader subdirectory
	_, added, err := cache.Add("grub", filepath.Join(d, "foobar"), "grubx64.efi")
	c.Assert(err, IsNil)
	c.Check(added, Equals, true)
	// sanity
...
func (s *assetsSuite) TestInstallObserverNew(c *C) {
	...
	obs, err := boot.NewTrustedAssetsInstallObserver(uc20Model, d)
	c.Assert(obs, NotNil)
	c.Assert(err, IsNil)

	// but nil for non UC20
	nonUC20Model := makeMockModel()
	nonUC20obs, err := boot.NewTrustedAssetsInstallObserver(nonUC20Model, d)
...
func (s *assetsSuite) TestInstallObserverObserveErr(c *C) {
	...
	// there is no known bootlaoder in gadget
	_, err = obs.Observe(gadget.ContentWrite, runBootStruct, boot.InitramfsUbuntuBootDir,
		filepath.Join(d, "foobar"), "EFI/boot/grubx64.efi")
