...
func ClearTryRecoverySystem(dev Device, systemLabel string) error {
	...
	resealErr := resealKeyToModeenv(dirs.GlobalRootDir, []*asserts.Model{dev.Model()}, m, expectReseal)
...
func SetTryRecoverySystem(dev Device, systemLabel string) (err error) {
	...
	const expectReseal = true
	return resealKeyToModeenv(dirs.GlobalRootDir, []*asserts.Model{dev.Model()}, m, expectReseal)
}
...
func PromoteTriedRecoverySystem(dev Device, systemLabel string, triedSystems []string) (err error) {
	...
	const expectReseal = true
	if err := resealKeyToModeenv(dirs.GlobalRootDir, []*asserts.Model{dev.Model()}, m, expectReseal); err != nil {
