...
func postFactoryResetCleanup() error {
	...
	// it is possible that the factory fallback key was already renamed
	saveFallbackKeyFactory := filepath.Join(InitramfsSeedEncryptionKeyDir, "ubuntu-save.recovery.sealed-key.factory-reset")
	saveFallbackKey := filepath.Join(InitramfsSeedEncryptionKeyDir, "ubuntu-save.recovery.sealed-key")
